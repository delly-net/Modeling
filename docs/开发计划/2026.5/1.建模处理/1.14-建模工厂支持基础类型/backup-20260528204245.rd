...
// 辅助方法：从建模集合中查找
#if NETSTANDARD2_0
private IEntityModel TryGetModelFromModelSets<T>() where T : class
#else
private IEntityModel? TryGetModelFromModelSets<T>() where T : class
#endif
{
...
- [ ] 重构 `GetModel<T>()` 方法支持基础类型（使用 typeof + switch case）
- [ ] 添加 `TryGetModelFromModelSets<T>()` 辅助方法
- [ ] 更新相关方法的 XML 文档注释
- [ ] 在 Deme 项目中添加示例代码
- [ ] 更新单元测试
- [ ] 验证所有基础类型建模正确注册
