...
- [ ] 移除 `RegisterBaseTypeModels()` 私有方法（如存在）
- [ ] 简化构造函数：移除对注册方法的调用
- [ ] 重构 `GetModel(string fullName)` 方法：优先判断基础类型全称，直接返回静态实例
- [ ] 重构 `GetModel<T>()` 方法：移除 `where T : class` 约束，使用 `typeof + switch case` 直接返回静态实例
- [ ] 重构 `TryGetModel<T>()` 方法：移除 `where T : class` 约束，使用 `typeof + switch case` 直接返回静态实例
