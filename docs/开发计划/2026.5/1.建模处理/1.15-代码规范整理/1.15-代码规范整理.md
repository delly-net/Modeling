...
// ❌ 错误 - 已超过 120 字符但仍为单行
if (veryLongConditionName && anotherVeryLongCondition) { ExecuteMethod(); }
```
...
| if 单行压缩 | if + 代码未超 120 字符但未压缩 | 压缩为单行 |
| if 多行折行 | if + 代码超 120 字符但未折行 | 折行为多行 |
| 私有注释 | 私有成员使用 `///` | 改为 `//` |
| 注释空格 | `//` 后无空格 | 添加空格 |
