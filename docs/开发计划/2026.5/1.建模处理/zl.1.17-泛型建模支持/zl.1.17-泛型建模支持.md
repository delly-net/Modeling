...
- **责任人**: 郑琳
- **任务类型**: 功能开发
- **任务描述**: 在 `IBaseModel` 提供泛型相关属性，在 `IModel` 提供泛型操作函数，以及常见泛型对象类

## 功能概述

在建模系统中添加泛型建模支持，包括：

1. **IsGenericModel 属性**：标识当前模型是否为泛型模型（定义在 `IBaseModel`）
2. **IsGenericDefinition 属性**：标识当前模型是否为开放泛型定义（定义在 `IBaseModel`）
3. **GenericDefinitionCount 属性**：存储泛型定义数量（定义在 `IBaseModel`）
4. **GetGenericModelDefinition 函数**：获取泛型模型的定义模型（定义在 `IModel`）
5. **GetGenericModels 函数**：获取所有已构造的泛型模型列表（定义在 `IModel`）
6. **MakeGenericModel 函数**：根据泛型参数创建已构造的泛型模型（定义在 `IModel`）
7. **常见泛型对象类**：`ListModel`、`DictionaryModel` 等预定义泛型定义模型
...
- 泛型相关方法（除 `IsGenericModel`、`IsGenericDefinition`、`GenericDefinitionCount` 外）定义在 `IModel` 接口
...
1. 在 `IBaseModel` 接口中添加 `IsGenericModel`、`IsGenericDefinition`、`GenericDefinitionCount` 属性定义
...
7. 源生成器为生成的模型类添加泛型相关属性支持

### 泛型模型语义

| 模型类型 | IsGenericModel | IsGenericDefinition | GenericDefinitionCount |
|:--------|:---------------|:--------------------|:-----------------------|
| 非泛型类型 (如 `int`) | `false` | `false` | `0` |
| 开放泛型定义 (如 `List<>`) | `true` | `true` | `1` |
| 已构造泛型 (如 `List<int>`) | `true` | `false` | `1` |
...
Console.WriteLine(listIntModel.IsGenericDefinition);    // false - 非开放泛型定义

var listDefinition = ListModel.Instance;
Console.WriteLine(listDefinition.IsGenericModel);       // true - 开放泛型定义也是泛型模型
Console.WriteLine(listDefinition.IsGenericDefinition);   // true - 开放泛型定义
...
| 源生成器 | 特性 | 生成的类 | 添加内容 |
|:--------|:----|:--------|:--------|
| `ModelableSourceGenerator` | `ModelableAttribute` | `emc__{ClassName}` | 泛型相关属性支持 |
| `ModelTableSourceGenerator` | `MoTableAttribute` | `emc__{ClassName}` | 泛型相关属性支持 |
| `ModelTableSourceGenerator` | `MoQueryAttribute` | `emc__{ClassName}` | 泛型相关属性支持 |
...
public partial class emc__User : IModel
{
    // ... 现有代码 ...
    ...
    /// <param name="models">泛型参数对应的模型列表</param>
    /// <returns>已构造的泛型模型</returns>
    /// <exception cref="NotSupportedException">User 不支持泛型建模创建</exception>
    public IModel MakeGenericModel(params IModel[] models)
    {
        throw new NotSupportedException("User 不支持泛型建模创建");
    }
}
```
...
Console.WriteLine(listIntModel.IsGenericDefinition);    // false
Console.WriteLine(listIntModel.GenericDefinitionCount); // 1

var listDefinition = ListModel.Instance;
Console.WriteLine(listDefinition.IsGenericModel);       // true
Console.WriteLine(listDefinition.IsGenericDefinition);   // true
Console.WriteLine(listDefinition.GenericDefinitionCount); // 1

var intModel = Int32Model.Instance;
Console.WriteLine(intModel.IsGenericModel);             // false
Console.WriteLine(intModel.IsGenericDefinition);        // false
Console.WriteLine(intModel.GenericDefinitionCount);     // 0
...
// 示例5：MakeGenericModel 参数校验
try
{
    // List<T> 只需要一个泛型参数
    ListModel.Instance.MakeGenericModel(Int32Model.Instance, StringModel.Instance);
}
...
try
{
    // Dictionary<TKey, TValue> 需要两个泛型参数
    DictionaryModel.Instance.MakeGenericModel(Int32Model.Instance);
}
catch (ArgumentException ex)
{
    Console.WriteLine(ex.Message);  // Dictionary<TKey, TValue> 需要两个泛型参数
}
```
...
- [ ] 在 `IBaseModel` 接口中添加 `IsGenericModel`、`IsGenericDefinition`、`GenericDefinitionCount` 属性定义
...
- [ ] 在 `ModelableSourceGenerator` 中生成泛型相关属性代码
- [ ] 在 `ModelTableSourceGenerator` 中添加泛型检测逻辑（MoTable）
- [ ] 在 `ModelTableSourceGenerator` 中生成泛型相关属性代码（MoTable）
- [ ] 在 `ModelTableSourceGenerator` 中添加泛型检测逻辑（MoQuery）
- [ ] 在 `ModelTableSourceGenerator` 中生成泛型相关属性代码（MoQuery）
- [ ] 添加 XML 文档注释
- [ ] 更新相关单元测试
- [ ] 在 Deme 项目中添加示例代码
- [ ] 验证 AOT 编译通过
...
1. `IBaseModel` 接口包含 `IsGenericModel`、`IsGenericDefinition`、`GenericDefinitionCount` 属性定义
...
9. 非泛型模型的 `IsGenericModel` 返回 `false`，`IsGenericDefinition` 返回 `false`，`GenericDefinitionCount` 返回 `0`
10. 非泛型模型的 `GetGenericModelDefinition()` 返回自身
11. 非泛型模型的 `GetGenericModels()` 返回空列表
12. 开放泛型定义（如 `ListModel.Instance`）的 `IsGenericModel` 返回 `true`，`IsGenericDefinition` 返回 `true`
13. 开放泛型定义的 `GetGenericModelDefinition()` 返回自身
14. 已构造泛型（如 `List<int>` 模型）的 `IsGenericModel` 返回 `true`，`IsGenericDefinition` 返回 `false`
...
19. `MakeGenericModel` 参数数量校验正确
20. 源生成器代码中不包含任何反射调用
21. 源生成器为生成的模型类添加泛型相关属性支持
22. XML 文档注释完整
23. 单元测试通过
24. Deme 项目中的示例能正常运行
25. AOT 编译通过
