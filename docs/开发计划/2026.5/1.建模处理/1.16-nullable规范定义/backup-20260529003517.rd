...
**Delly.Modeling/Delly.Modeling.csproj** 修改（多目标框架项目）：
```xml
<PropertyGroup>
  <TargetFrameworks>netstandard2.0;net6.0</TargetFrameworks>
...
**注意**：多框架项目配置 `<Nullable>enable</Nullable>` 时需要加上指定框架兼容性适配条件（具体实现方式待确认）。

**Delly.Modeling.Generator/Delly.Modeling.Generator.csproj** 保持不变（纯 .NET Standard 2.0 项目）：
```xml
<PropertyGroup>
  <TargetFramework>netstandard2.0</TargetFramework>
  <LangVersion>latest</LangVersion>
...
**注意**：纯 .NET Standard 2.0 项目不需要配置 `<Nullable>enable</Nullable>`。
...
**解决方案**：
1. 多目标框架项目（如 Delly.Modeling）使用 `<Nullable>enable</Nullable>`，net6.0 目标生效，netstandard2.0 目标忽略
2. 纯 .NET Standard 2.0 项目（如 Delly.Modeling.Generator）不需要配置 `<Nullable>enable</Nullable>`
...
**1.1 修改 Delly.Modeling/Delly.Modeling.csproj**（多目标框架项目）
- 在 `<PropertyGroup>` 中添加 `<Nullable>enable</Nullable>`
- 位置：在 `<TargetFrameworks>` 之后
- **注意**：需要加上指定框架兼容性适配条件（具体实现方式待确认）

**1.2 Delly.Modeling.Generator/Delly.Modeling.Generator.csproj**（纯 .NET Standard 2.0 项目）
- 不需要修改，保持项目文件简洁
...
- [ ] **确认源生成器生成的代码是否需要保留 nullable 指令**
  - **建议**：保留，确保生成代码的自包含性和正确性
    - 保留
- [ ] **确认是否需要为不同目标框架设置不同的 nullable 配置**
  - **建议**：不需要，统一使用 `<Nullable>enable</Nullable>` 即可
    - 根据目标框架兼容性来设定
- [ ] **NEW: 确认多框架项目配置 `<Nullable>enable</Nullable>` 时的"框架兼容性适配条件"具体实现方式**
  - **需要澄清**：具体是什么条件、如何实现、是否需要在 csproj 中添加条件编译

## 使用示例

### 修改前后对比

#### csproj 文件对比 - Delly.Modeling.csproj
...
**注意**：上述配置需要根据确认的"框架兼容性适配条件"进行调整。

#### csproj 文件对比 - Delly.Modeling.Generator.csproj

**修改前后保持不变**（纯 .NET Standard 2.0 项目不需要配置 nullable）：
```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <TargetFramework>netstandard2.0</TargetFramework>
...
- [ ] **csproj 文件修改**
  - [ ] Delly.Modeling/Delly.Modeling.csproj - 添加 `<Nullable>enable</Nullable>`（框架兼容性适配条件待确认）
  - [ ] Delly.Modeling.Generator/Delly.Modeling.Generator.csproj - 保持不变（纯 .NET Standard 2.0）
...
1. `Delly.Modeling.csproj` 包含 `<Nullable>enable</Nullable>` 配置（含框架兼容性适配条件）
2. `Delly.Modeling.Generator.csproj` 保持不变（纯 .NET Standard 2.0 项目不需要配置）
