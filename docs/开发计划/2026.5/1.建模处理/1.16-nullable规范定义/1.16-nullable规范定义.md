...
**解决方案**：
1. 多目标框架项目（如 Delly.Modeling）使用带 MSBuild Condition 的 `<Nullable>enable</Nullable>` 配置
...
- 添加带 Condition 的 PropertyGroup 配置 nullable
- 使用 `Condition="'$(TargetFramework)'!='netstandard2.0'"` 确保仅对支持的框架启用
...
- [x] **确认多框架项目配置 `<Nullable>enable</Nullable>` 的"框架兼容性适配条件"实现方式**（已确认：使用 MSBuild Condition）
...
- [ ] **csproj 文件修改**
  - [ ] Delly.Modeling/Delly.Modeling.csproj - 添加带 MSBuild Condition 的 `<Nullable>enable</Nullable>` 配置
