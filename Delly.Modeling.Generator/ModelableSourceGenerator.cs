...
public class ModelableSourceGenerator : ISourceGenerator
{
    ...
    private static string GenerateSourceCode(string namespaceName, string className, List<IPropertySymbol> properties)
    {
        ...
        sb.AppendLine("    /// 获取模型");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine($"    public static c__{className}Model GetModel() => c__{className}Model.Instance;");
        sb.AppendLine("}");
        ...
        foreach (var property in properties)
        {
            ...
            sb.AppendLine("/// 自动模型");
            sb.AppendLine("/// </summary>");
            var propertyClassName = $"p__{className}Model_{property.Name}";
            ...
            sb.AppendLine("    /// 属性建模");
            sb.AppendLine("    /// </summary>");
            sb.AppendLine($"    public IModel PropertyModel => {GetModelClassName(property.Type)}.Instance;");
            sb.AppendLine();
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// 获取值");
            sb.AppendLine("    /// </summary>");
        ...
        sb.AppendLine("/// 自动模型");
        sb.AppendLine("/// </summary>");
        sb.AppendLine($"public class c__{className}Model : IModel");
        sb.AppendLine("{");
        sb.AppendLine();
        sb.AppendLine("    // 固定静态共享实例");
        sb.AppendLine($"    private static readonly c__{className}Model _instance = new c__{className}Model();");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// 公共实例");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine($"    public static c__{className}Model Instance => _instance;");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// 名称");
        sb.AppendLine("    /// </summary>");
        ...
        sb.AppendLine("    {");
        sb.Append("        return [");
        var propertyModelClasses = properties.Select(p => $"p__{className}Model_{p.Name}.Instance");
        ...
        foreach (var property in properties)
        {
            ...
            sb.AppendLine($"                return p__{className}Model_{property.Name}.Instance;");
        }
